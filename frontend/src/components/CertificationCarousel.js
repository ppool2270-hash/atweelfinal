...
export const CERTIFICATION_ITEMS = [
  {
    ...
    badgeColor: "from-tata-dark to-tata-dark",
    accentColor: "#10B981"
  },
  {
    id: "usda-organic",
...
export default function CertificationCarousel({ onNavigateTab }) {
  ...
  return (
    <section 
      className="py-16 sm:py-20 bg-gradient-to-b from-tata-bg-light via-tata-blue-dark/40 to-tata-bg-light border-y border-white/20 text-tata-dark-800 relative overflow-hidden"
      data-testid="certification-carousel-section"
    >
      {/* Background Decorative Accents */}
      <div className="absolute -top-24 -right-24 w-96 h-96 bg-tata-bg-light rounded-full blur-3xl pointer-events-none" />
      <div className="absolute -bottom-24 -left-24 w-96 h-96 bg-tata-dark/10 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10 sm:mb-12">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-tata-dark text-tata-blue-light border border-white/20 text-xs font-bold uppercase tracking-[0.2em] shadow-sm mb-3">
            <ShieldCheck className="w-4 h-4 text-tata-dark" />
          ...
          <p className="text-tata-dark-600 text-sm sm:text-base mt-3 leading-relaxed">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all cursor-pointer flex items-center gap-1.5 ${
                activeCategory === cat
                  ...
                  : "bg-white text-tata-dark hover:bg-tata-blue-light border border-tata-grey-light"
              }`}
            >
              {cat === "Organic" && <Leaf className="w-3.5 h-3.5 text-tata-grey" />}
              {cat === "Food Safety" && <ShieldCheck className="w-3.5 h-3.5 text-tata-grey" />}
              {cat === "Regulatory" && <Building className="w-3.5 h-3.5 text-tata-grey" />}
              {cat === "Lab Testing" && <Award className="w-3.5 h-3.5 text-tata-grey" />}
              <span>{cat}</span>
        ...
        >
          ...
          <div className="bg-white border-2 border-white/20 rounded-3xl p-6 sm:p-8 lg:p-10 shadow-xl relative overflow-hidden transition-all duration-500">
            {/* Top Accent Stripe */}
            <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-tata-dark via-tata-blue-dark to-tata-dark" />

            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              
              {/* Left Column: Emblem / Badge Display */}
              <div className="lg:col-span-5 flex flex-col items-center text-center p-6 rounded-2xl bg-gradient-to-b from-tata-dark to-tata-dark text-white border border-white/20 relative shadow-inner">
                
                {/* Active Verified Pill */}
                <div className="absolute top-4 right-4 bg-tata-bg-light/20 text-tata-grey border border-tata-grey-light/40 text-[10px] font-mono uppercase tracking-wider px-2.5 py-1 rounded-full flex items-center gap-1.5 font-bold">
                ...
                <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl bg-white/10 border-2 border-white/20 flex items-center justify-center my-4 p-4 shadow-lg backdrop-blur-sm">
                  {activeCert.id === "npop" || activeCert.id === "usda-organic" || activeCert.id === "eu-organic" ? (
                    <Leaf className="w-12 h-12 text-tata-dark" />
                  ) : activeCert.id === "fssai" ? (
                    <Building className="w-12 h-12 text-tata-grey" />
                  ) : activeCert.id === "haccp" || activeCert.id === "iso-22000" ? (
                    <ShieldCheck className="w-12 h-12 text-tata-grey" />
                  ) : (
                    <Award className="w-12 h-12 text-tata-grey" />
                  )}
                </div>

                {/* Short Name & Category */}
                <span className="text-[11px] font-bold uppercase tracking-widest text-tata-dark bg-black/30 px-3 py-1 rounded-full border border-white/20 mb-2">
                ...
                <p className="text-tata-grey text-xs font-mono">
                ...
                <div className="mt-5 w-full pt-4 border-t border-white/15 flex flex-col items-center gap-2">
                  <span className="text-[10px] text-tata-grey uppercase tracking-wider">Official Certificate / Reg No:</span>
                  <button
                    type="button"
                    onClick={() => handleCopyCertId(activeCert.certId)}
                    data-testid={`copy-cert-id-${activeCert.id}`}
                    className="w-full bg-white/10 hover:bg-white/20 text-tata-dark border border-white/20 px-3 py-2 rounded-xl text-xs font-mono font-bold transition-all flex items-center justify-center gap-2 cursor-pointer"
                  >
                    {copiedCertId === activeCert.certId ? (
                      <>
                        <Check className="w-4 h-4 text-tata-grey" />
                        <span className="text-tata-grey">Copied to Clipboard!</span>
                      </>
                    ) : (
                      <>
                        <Copy className="w-3.5 h-3.5 text-tata-dark" />
              ...
              <div className="lg:col-span-7 space-y-5">
                <div>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="text-xs font-bold uppercase tracking-wider text-tata-dark">
                      Certification Scope &amp; Audit Profile
                    </span>
                    <span className="text-xs text-tata-grey font-mono">
                      Valid: <strong className="text-tata-dark">{activeCert.validUntil}</strong>
                    </span>
                  </div>
                  <h3 className="font-serif text-2xl sm:text-3xl font-bold text-tata-dark mt-1">
                    {activeCert.fullName}
                  </h3>
                ...
                <div className="bg-tata-bg-light border border-white/20 rounded-2xl p-4 sm:p-5">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-tata-dark block mb-3">
                    Institutional Audit Highlights:
                  </span>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {activeCert.highlights.map((h, hIdx) => (
                      <div key={hIdx} className="flex items-center gap-2 text-xs font-medium text-tata-dark">
                ...
                <div className="text-xs bg-tata-bg-light text-tata-dark border border-tata-grey-light rounded-xl p-3 font-mono leading-snug">
                  <span className="font-bold text-tata-dark">Auditing Authority: </span>
                  {activeCert.issuer}
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                  <button
                    ...
                    className="inline-flex items-center gap-2 bg-tata-dark hover:bg-tata-dark text-tata-blue-light font-bold px-4 py-2.5 rounded-xl text-xs uppercase tracking-wider transition-all shadow-sm cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 text-xs text-tata-dark font-bold hover:text-tata-dark hover:underline cursor-pointer"
                  >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="w-8 h-8 rounded-full bg-white border border-tata-grey-light flex items-center justify-center text-tata-dark hover:text-tata-dark hover:border-tata-dark/20 transition-all cursor-pointer shadow-sm"
                title={isPaused ? "Resume Auto Slide" : "Pause Auto Slide"}
              >
                {isPaused ? <Play className="w-3.5 h-3.5 text-tata-grey" /> : <Pause className="w-3.5 h-3.5" />}
              </button>

              <span className="text-xs font-mono text-tata-grey">
                Certification <strong className="text-tata-dark">{currentIndex + 1}</strong> of <strong className="text-tata-dark">{filteredItems.length}</strong>
              </span>
            </div>
              ...
              {filteredItems.map((item, idx) => (
                <button
                  ...
                  className={`h-2.5 rounded-full transition-all cursor-pointer ${
                    currentIndex === idx 
                      ? "w-8 bg-tata-dark" 
                      : "w-2.5 bg-tata-bg-light hover:bg-tata-blue-light"
                  }`}
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-2.5 rounded-xl bg-white border border-tata-grey-light text-tata-dark hover:bg-tata-dark hover:text-tata-blue-light hover:border-tata-dark transition-all cursor-pointer shadow-sm"
        ...
        <div className="mt-14 pt-10 border-t border-white/20 max-w-5xl mx-auto">
          <span className="text-center block text-[11px] font-bold uppercase tracking-[0.25em] text-tata-grey mb-6">
              ...
              <button
                ...
                className={`p-3 rounded-xl border text-center transition-all cursor-pointer ${
                  currentIndex === index && activeCategory === "All"
                    ? "bg-tata-dark text-tata-blue-light border-tata-dark/20 shadow-md ring-2 ring-tata-blue-dark/30"
                    : "bg-white text-tata-dark border-tata-grey-light hover:border-white/20 hover:bg-tata-bg-light"
                }`}
              >
      ...
      {selectedCertModal && (
        ...
        >
          <div className="bg-white border-2 border-tata-dark/20 rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl relative my-8">
            {/* Close Modal Button */}
            <button
              onClick={() => setSelectedCertModal(null)}
              data-testid="close-cert-modal-btn"
              className="absolute top-4 right-4 w-9 h-9 rounded-full bg-tata-bg-light hover:bg-tata-dark hover:text-tata-blue-light flex items-center justify-center text-tata-grey transition-all cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex items-start gap-4 pr-10">
              <div className="w-12 h-12 rounded-2xl bg-tata-dark text-tata-blue-light flex items-center justify-center shrink-0 border border-white/20 shadow-md">
                <ShieldCheck className="w-6 h-6" />
              </div>
              <div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-tata-dark bg-tata-bg-light px-2.5 py-0.5 rounded border border-white/20">
                  {selectedCertModal.category} Standard
                </span>
                <h3 className="font-serif text-2xl font-bold text-tata-dark mt-1">
            ...
            <div className="mt-6 space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-tata-bg-light p-4 rounded-2xl border border-white/20">
                <div>
                  <span className="text-tata-grey block font-mono">Certificate / License ID:</span>
                  <span className="font-mono font-bold text-tata-dark text-sm">{selectedCertModal.certId}</span>
                </div>
                <div>
                  <span className="text-tata-grey block font-mono">Status:</span>
                  <span className="font-bold text-tata-grey flex items-center gap-1">
                    <CheckCircle2 className="w-3.5 h-3.5 text-tata-grey" />
                    Verified Active ({selectedCertModal.validUntil})
                  </span>
                </div>
                <div>
                  <span className="text-tata-grey block font-mono">Issued Date:</span>
                  <span className="font-medium text-tata-dark">{selectedCertModal.issued}</span>
                </div>
                <div>
                  <span className="text-tata-grey block font-mono">Issuing Authority:</span>
                  <span className="font-medium text-tata-dark">{selectedCertModal.issuer}</span>
                </div>
              </div>

              <div>
                <h4 className="font-bold text-tata-dark mb-1">Accreditation Body &amp; Standards:</h4>
                <p className="text-tata-grey leading-relaxed bg-tata-bg-light p-3 rounded-xl border border-tata-grey-light">
                  {selectedCertModal.accreditation}
                </p>
              </div>

              <div>
                <h4 className="font-bold text-tata-dark mb-1">Full Certified Scope:</h4>
                <p className="text-tata-grey leading-relaxed bg-tata-bg-light p-3 rounded-xl border border-tata-grey-light">
              ...
              <div>
                <h4 className="font-bold text-tata-dark mb-1">Audit Protocol &amp; Quality Findings:</h4>
                <p className="text-tata-dark font-medium leading-relaxed bg-tata-bg-light text-tata-dark p-3 rounded-xl border border-tata-grey-light">
            ...
            {/* Modal Actions */}
            <div className="mt-6 pt-4 border-t border-tata-grey-light flex flex-wrap items-center justify-between gap-3">
              <button
                type="button"
                onClick={() => handleCopyCertId(selectedCertModal.certId)}
                className="bg-tata-bg-light hover:bg-tata-bg-light text-tata-dark font-bold px-4 py-2 rounded-xl text-xs flex items-center gap-2 cursor-pointer"
              >
                {copiedCertId === selectedCertModal.certId ? <Check className="w-4 h-4 text-tata-grey" /> : <Copy className="w-4 h-4" />}
              ...
              <button
                type="button"
                onClick={() => setSelectedCertModal(null)}
                className="bg-tata-dark hover:bg-tata-dark text-tata-blue-light font-bold px-5 py-2 rounded-xl text-xs cursor-pointer"
              >
                Close Audit Viewer
              </button>
            </div>
